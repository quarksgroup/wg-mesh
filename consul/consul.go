...
func New(endpoint string) (*Consul, error) {
	...
	return &Consul{client: ConsulClient}, nil
}
...
func (cb *Consul) GetPeers(location string) ([]wireguard.Peer, error) {
	...
	for _, consulPeer := range consulPeers {
		...
		exp, _ := regexp.Compile(`^.+\/peers\/[0-9\.]+$`)

		isKeyPeer := exp.MatchString(consulPeer.Key)
...
func (cb *Consul) monitorKvPrefix(location string, newPeersChan chan []wireguard.Peer, stopMonitorKvPrefixChan chan bool) {
	ConsulKV := cb.client.KV()
...
func (cb *Consul) monitorNodes(location string, wgInterface wireguard.Interface, newNodesChan chan []string, stopMonitorNodesChan chan bool) {
	opts := &api.LockOptions{
...
func (cb *Consul) removeLeftPeers(location string, newNodes []string) {
	ConsulKV := cb.client.KV()
