...
func InitWgKeys(wgInterfaceConfigFolder string) (string, string, error) {
	if _, err := os.Stat(wgInterfaceConfigFolder + "/private"); os.IsNotExist(err) {
		...
		err = os.WriteFile(wgInterfaceConfigFolder+"/private", privKey, 0600)
		if err != nil {
			return "", "", err
		}
	...
	privKey, err := os.ReadFile(wgInterfaceConfigFolder + "/private")
	if err != nil {
		return "", "", err
	}
...
func ConfigureWireguard(wgConfig Configuration) ([]byte, error) {
	...
	if isWGInterfaceStarted {
		...
		if err != nil {
			return nil, fmt.Errorf("error Syncing WireGuard Peers: %v", err)
		}
		return result, nil
		...
		if err != nil {
			return nil, fmt.Errorf("error Bringing Up the Wireguard Interface: %v", err)
		}
		return result, nil
	}
}
...
func isInterfaceAlreadyConfigured(wgInterface Interface) bool {
	...
	return currentWgPort == wgInterface.ListenPort
